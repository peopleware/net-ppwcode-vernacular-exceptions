...
[NOTE]
====
As we will describe further on in the text, using the correct exception (base) types makes it easier to triage thrown exceptions.  It also makes it easier to have common code, for example to triage exceptions and translate them into an HTTP status code in an `Asp.NET Core` application.
====
...
[TIP]
====
Instead of directly throwing `ProgrammingError` exceptions, you could use the library `PPWCode.Vernacular.Contracts.I` that contains helper code to express assumptions inside your code in a _Design By Contract_ style with preconditions, postconditions, invariants and assertions.  See xref:ppwcode-vernacular-contracts-i:ROOT:index.adoc[PPWCode.Vernacular.Contracts.I].
====
...
`CompoundSemanticException` is a collection of `SemanticException` instances and is used to bundle all `SemanticException`s for a given input into one exception.
